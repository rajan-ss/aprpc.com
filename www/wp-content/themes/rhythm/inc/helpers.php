...
function ts_get_embaded_video($url, $return_url = false) {
	...
	if (strstr($url,'vimeo'))
	{
		...
		{
			if ($return_url) {
				return 'https://player.vimeo.com/video/'.$matches[0].'?title=0&amp;byline=0&amp;portrait=0&amp;color=ffffff';
			}
			return '<iframe src="https://player.vimeo.com/video/'.$matches[0].'?title=0&amp;byline=0&amp;portrait=0&amp;color=ffffff"></iframe>';
		}
	}
	else if (strstr($url,'youtube'))
	{
		...
		if (preg_match($pattern, $url, $matches))
		{
			...
			return '<iframe src="https://www.youtube.com/embed/'.$matches[0].'"></iframe>';
		}
	}
	return '';
}

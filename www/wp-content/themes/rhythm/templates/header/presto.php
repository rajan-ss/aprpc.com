...
<?php if (ts_get_opt('header-enable-switch') == 1): ?>
	<!-- Navigation panel -->
	<nav class="main-nav main-nav-style2 <?php echo sanitize_html_classes(implode(' ',$header_class));?>">
		<div class="<?php echo (ts_get_opt('header-full-width') == 1 ? 'full-wrapper' : 'container' ) ?> relative clearfix">
			<div class="nav-logo-wrap local-scroll">
				<?php rhythm_logo('logo', get_template_directory_uri().'/images/logo-presto.png'); ?>
				<?php rhythm_logo('logo-light', get_template_directory_uri().'/images/logo-presto-alt.png'); ?>
			</div>
				...
				<ul class="clearlist modules scroll-nav local-scroll">
					...
					<?php if (ts_get_opt('header-enable-social-icons')): ?>
							...
							<li>
								<?php 
									if (ts_get_opt('header-style') == 'dark') {
										rhythm_social_links('%s',ts_get_opt('header-social-icons-category'), 4); 
									} 
									else {
										rhythm_social_links('%s',ts_get_opt('header-social-icons-category'), 5);
									} 
								?>
							</li>
							<!-- End Social Links -->
